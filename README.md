...
Refer to `docs/components.html` for real-world markup examples that exercise each component and match the Bootstrap data-API triggers. A larger AdminLTE 2-inspired migration example family starts at `examples/admin-panel/index.html`, with generated split pages for widgets, mailbox, forms, advanced forms, plugin bridging, users, ecommerce, reports, tables, profile, invoice, calendar, buttons, modals, timeline, login, and errors. `examples/admin-panel/cdn-starter.html` is the quick no-build variant: it uses `@tailwindcss/browser@4`, inline `@theme` tokens, and Bootstrap JS from CDN. `examples/admin-panel/plugins.html` demonstrates scoped optional Slim Select and Tom Select integration for legacy plugin screens without jQuery. The generated examples keep Bootstrap JS interactions while using Windform/Tailwind tokens instead of AdminLTE assets. Run `npm run build:admin` to regenerate both source examples and the Pages copy under `docs/examples/admin-panel/`; the production stylesheet scans `./examples/**/*.html`, so run `npm run build` before opening a source checkout directly. On GitHub Pages, open `https://md-riaz.github.io/Windform/examples/admin-panel/`.
